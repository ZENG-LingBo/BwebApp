...
export default function BottomBar({
  ...
  return (
    <div className="bottom-bar">
      {/* Refresh button */}
      <button className="icon-btn" onClick={onRefresh} disabled={refreshing} title="Refresh stories">
      ...
      <div className="story-nav">
        <button
          className="nav-arrow"
          onClick={onPrev}
          disabled={!hasStories || currentIdx === 0}
        >
        ...
        <button className="next-story-btn" onClick={onNext} disabled={!hasStories || currentIdx >= storyCount - 1}>
          <span className="next-story-label">
            {hasStories ? `${currentIdx + 1} / ${storyCount}` : 'No stories'}
          </span>
        </button>
        ...
        <button
          className="nav-arrow"
          onClick={onNext}
          disabled={!hasStories || currentIdx >= storyCount - 1}
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
